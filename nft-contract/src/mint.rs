...
impl Contract {
    ...
    pub fn nft_mint(
        ...
        //check if token is on allowed list
        //metadata: id, account, asset

        /*if let Some(ref token_type) = token_type {
            final_token_id = format!("{}.{}",token_type,self.tokens_per_type.get(&token_type).try_to_vec().unwrap().len() + 1);
            //final_token_id = format!("{}.{}",token_type, final_token_id);
        }*/
        ...
        if let Some(perpetual_royalties) = perpetual_royalties {
            ...
            for (account, amount) in perpetual_royalties {
                ...
                if token_type.is_some() {
                    ...
                    final_token_id = format!("{}.{}",token_type,u64::from(self.nft_supply_for_type(&token_type)) + 1);
